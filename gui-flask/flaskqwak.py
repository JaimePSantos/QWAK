...
def staticQW():
    ...
    if not session.get('staticQwakId'):
        ...
        probDistSessionCollection.insert_one(json.loads(staticQWAK.to_json()))
...
def dynamicQW():
    ...
    if not session.get('dynamicQwakId'):
        ...
        dynamicQwak = QWAK(graph=dynamicGraph, qwakId=session['dynamicQwakId'])
...
def setRunWalkDBTest():
    if request.method == 'POST':
        ...
        probDistSessionCollection.replace_one({'qwakId': sessionId}, json.loads(staticQWAK.to_json()))
    return ("nothing")
...
def getRunWalkDBTest():
    if request.method == 'POST':
        probDistSessionCollection = database[session['sessionId']]
        sessionId = session['sessionId']
        staticQWAK = QWAK.from_json(probDistSessionCollection.find_one({'qwakId': sessionId}))
...
def setRunMultipleWalksDBTest():
    if request.method == 'POST':
        ...
        dynamicQWAK = QWAK.from_json(probDistSessionCollection.find_one({'qwakId': session['dynamicQwakId']}),True)
