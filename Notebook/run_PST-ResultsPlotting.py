...
def plot_dual_y_axis(params):
    ...
    for i, (first_half_values, second_half_values) in enumerate(value_pairs):
        ...
        ax2.set_ylabel(f"Transfer Vertex", color='green', fontsize=ylabel_font_size)
        ax2.tick_params(axis="y", labelcolor='green', labelsize=tick_font_size)
    ...
    ax1.tick_params(axis="y", labelcolor='blue', labelsize=tick_font_size)
    ax1.tick_params(axis="x", labelsize=tick_font_size)

    plt.title("Hypercube Perfect State Transfer", fontsize=title_font_size)
