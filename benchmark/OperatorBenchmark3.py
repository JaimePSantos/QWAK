...
from qwak.Operator import Operator  # Assuming this is your custom import
...
class OperatorBenchmark3:
    ...
    def buildDiagonalOperatorEigTimed(self, graph: nx.Graph, laplacian: bool = False, markedSearch=None, time: float = 0) -> float:
        start_time = timer()
        self._eigenvalues, self._eigenvectors = np.linalg.eigh(
            self._adjacencyMatrix)
        diag = np.diag(
    ...
    def buildDiagonalOperatorNoEigTimed(self, graph: nx.Graph, laplacian: bool = False, markedSearch=None, time: float = 0) -> float:
        start_time = timer()
        diag = np.diag(
            np.exp(-1j * self._eigenvalues * time)).diagonal()
        self._operator = np.multiply(self._eigenvectors, diag)
    ...
    def buildSlowDiagonalOperatorTimed(self, graph: nx.Graph, laplacian: bool = False, markedSearch=None, time: float = 0) -> float:
        start_time = timer()
        self._adjacencyMatrix = np.asarray(
    ...
    def buildSlowDiagonalOperatorNoAdjTimed(self, graph: nx.Graph, laplacian: bool = False, markedSearch=None, time: float = 0) -> float:
        start_time = timer()
        self.slowperator = ln.expm(-1j * self._adjacencyMatrix * time)
        end_time = timer()
        return end_time - start_time
