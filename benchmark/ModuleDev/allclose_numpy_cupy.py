...
def create_results_folder(nlist, time):
    ...
    folder_name = f"AllCloseDatasets_Numpy-Cupy_n{
...
def plot_final_states(n, results_folder):
    ...
    numpy_prob_vec_file = os.path.join(
        n_folder, f"numpy_prob_vec_n{n}.txt")
    cupy_prob_vec_file = os.path.join(
        n_folder, f"cupy_prob_vec_n{n}.txt")
...
def main(nlist, time):
    ...
    for n in tqdm(nlist, desc="Processing graph sizes"):
        ...
        comparison_result = compare_final_states(
            final_state_numpy, final_state_cupy)
