...
def run_qwak_walk_normal_mode(graph_size):
    graph = nx.erdos_renyi_graph(graph_size, p=0.8, seed=10)
    qwak_instance = QWAK(graph)
    qwak_instance.runWalk(time=10.0, initStateList=[graph_size//2])
    final_state_normal = qwak_instance.getProbVec()
    return final_state_normal

def run_qwak_walk_expm_mode(graph_size):
    graph = nx.erdos_renyi_graph(graph_size, p=0.8, seed=10)
    qwak_instance = QWAK(graph)
    qwak_instance.runExpmWalk(time=10.0, initStateList=[graph_size//2])
...
def create_results_folder(nlist):
    ...
    folder_name = f"AllCloseDatasets_Eigh-Expm_n{min(nlist)}-{max(nlist)}"
...
def plot_final_states(n, results_folder):
    ...
    eigh_prob_vec_file = os.path.join(n_folder, f"eigh_prob_vec_n{n}.txt")
    expm_prob_vec_file = os.path.join(n_folder, f"expm_prob_vec_n{n}.txt")
...
def main(nlist):
    ...
    for n in tqdm(nlist, desc="Processing graph sizes"):
        ...
        if not os.path.exists(eigh_prob_vec_file):
            final_state_normal = run_qwak_walk_normal_mode(n)
        ...
        if not os.path.exists(expm_prob_vec_file):
            final_state_expm = run_qwak_walk_expm_mode(n)
            np.savetxt(expm_prob_vec_file, final_state_expm)
        else:
            final_state_expm = np.loadtxt(expm_prob_vec_file)
        
        comparison_result = compare_final_states(final_state_normal, final_state_expm)
...
if __name__ == "__main__":
    nlist = range(3, 500)  # Graph sizes from 3 to 1000
    main(nlist)
