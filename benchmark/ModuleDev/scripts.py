...
def run_if_not_profiled(instance, method_name: str, **kwargs):
    ...
    # If 'graph' is provided, update 'n' accordingly since 'n' is used
    # in the filename.
    if 'graph' in kwargs:
    ...
    try:
        ...
        print(f"Profiling file '{existing_file}' already exists for method '{
              method_name}' with sample {instance.sample}. Skipping execution.")
    except FileNotFoundError:
        print(f"Profiling file not found for method '{
              method_name}' with sample {instance.sample}. Running execution.")
        method(**kwargs)


def create_profiling_data_cycle(
        n_values: List[int],
        sample_range=range(
            0,
            3),
...
def create_profiling_data_ER_HPC(
        n_values: List[int],
        sample_range=range(
            0,
            3),
...
def create_profiling_data_hiperwalk(
    n_values: List[int], sample_range=range(
        0, 3), t: int = 10, pVal=0.8, seed=10):
    """
    Runs profiling for a variable number of n's, ensuring that profiling files are created if they do not exist.
...
def run_if_not_profiled_time_range(
        instance,
        method_name: str,
        time_range: List[int],
        **kwargs):
    ...
    for t in time_range:
        ...
        # If 'graph' is provided, update 'n' accordingly since 'n' is
        ...
        try:
            existing_file = find_exact_profiling_file(instance, method)
            print(f"Profiling file '{existing_file}' already exists for method '{
                  method_name}' with time {t} and sample {instance.sample}. Skipping execution.")
        except FileNotFoundError:
            print(f"Profiling file not found for method '{method_name}' with time {
                  t} and sample {instance.sample}. Running execution.")
            method(**kwargs)


def create_profiling_data_hiperwalk_hpc(
    n_values: List[int], sample_range=range(
        0, 3), t: int = 10, pVal=0.8, seed=10):
    for n in n_values:
        print(f"Creating profiling data for n = {n}")
        bench = HiperwalkBenchmark_HPC(
            tracked_attributes=[
                'n', 'sample', 'pVal', 'seed'])
        graph = nx.erdos_renyi_graph(n, pVal, seed=seed)
...
def create_profiling_data_animation(
    time_range: List[int], sample_range=range(
        0, 3), n: int = 10, pVal=0.8, seed=10):
    ...
    for n in tqdm(nrange, desc="Processing n-values"):
        ...
        for sample in sample_range:
            ...
            with open(filepath, 'r') as f:
                ...
                if not found:
                    raise ValueError(
                        f"Method '{method_name}' not found in {filepath}")
...
def git_branch_commit_push(branch_name, commit_message):
    ...
    try:
        ...
        subprocess.check_call(
            ['git', 'commit', '-m', "PYTHON:" + commit_message])
