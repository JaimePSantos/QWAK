...
class HiperwalkBenchmark_HPC:
    ...
    @profile(
        output_path="hiperwalk_results_hpc",
        sort_by="cumulative",
        lines_to_print=None,
        strip_dirs=False,
        csv=False,
    ...
    def init_hiperwalk(
        ...
        if sample is not None:
            self.sample = sample
        self.hiperwalk = hpw.ContinuousTime(
            graph=self.graph, gamma=1 / (2 * np.sqrt(2)), time=time)
    ...
    def simulate(self, time=0, sample=None, pVal=0.8, seed=10):
        if sample is not None:
            self.sample = sample
        self.final_state = self.hiperwalk.simulate(
            range=(self.n // 2, (self.n // 2) + 1), state=self.state)
