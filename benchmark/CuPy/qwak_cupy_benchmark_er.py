...
#    print(f"Running NP QWAK {len(nList)} walks up to n = {nList[-1]}")
    for n, adjList in tqdm(zip(nList, adjList_list), desc=f"NPQWAK {
                           len(nList)}:{nList[0]}->{nList[-1]}", leave=False):
        for sample, adj in tqdm(
                zip(range(1, samples + 1), adjList), desc=f"Samples for N = {n}"):
            # print(f"----> Calculating NP QWAK for n = {n} \t Sample
            # #{sample}",end='\r')
...
#    print(f"Running CuPy QWAK {len(nList)} walks up to n = {nList[-1]}")
    for n, adjList in tqdm(zip(nList, adjList_list), desc=f"CPQWAK {
                           len(nList)}:{nList[0]}->{nList[-1]}", leave=False):
        for sample, adj in tqdm(
                zip(range(1, samples + 1), adjList), desc=f"Samples for N = {n}"):
            # print(f"----> Calculating CuPy QWAK for n = {n} \t Sample
            # #{sample}",end='\r')
...
def create_or_generate_adjacency_matrices(
        base_dir, n_values, pVal, samples):
    """
    ...
    for n in tqdm(n_values, desc="Processing graph sizes"):
        ...
        for sample in range(samples):
            ...
            if not os.path.exists(sample_file):
                # Generate and save a new adjacency matrix if it does
                # not exist
                graph = nx.erdos_renyi_graph(n, pVal)
                adjacency_matrix = nx.to_numpy_array(graph)
                with open(sample_file, 'wb') as f:
            ...
            # Add the path to the list (only paths are stored in memory
            # here)
...
def runMultipleSimpleQWAK2(nList, t, samples, adjList_paths):
    ...
    for n, adj_paths in tqdm(zip(nList, adjList_paths.values()), desc=f"NPQWAK {
                             len(nList)}:{nList[0]}->{nList[-1]}", leave=False):
        for sample_idx, adj_path in tqdm(enumerate(
                adj_paths, start=1), desc=f"Samples for N = {n}", leave=False):
...
def runMultipleSimpleQWAK_cupy2(nList, t, samples, adjList_paths):
    ...
    for n, adj_paths in tqdm(zip(nList, adjList_paths.values()), desc=f"CPQWAK {
                             len(nList)}:{nList[0]}->{nList[-1]}", leave=False):
        for sample_idx, adj_path in tqdm(enumerate(
                adj_paths, start=1), desc=f"Samples for N = {n}", leave=False):
            # Load adjacency matrix only when needed
...
# , seed_list_dict):
def runMultipleSimpleQWAK3(nList, pVal, t, samples):
    qwList = []
    tList = []
    qwak_time = 0
    qwak_time_average = 0
...
def runMultipleSimpleQWAK3_cupy(
        nList, pVal, t, samples):  # , seed_list_dict):
    qwList = []
    tList = []
    qwak_time = 0
    qwak_time_average = 0

    for n in tqdm(nList, desc=f"CuPyQWAK {len(nList)}:{
                  nList[0]}->{nList[-1]}", leave=False):
...
qwak_times_filename = f'simpleQWAKTime_N{
    nMin}-{nMax - 1}_P{pVal}_T{t}_S{samples}.txt'
qwak_times_filename_cupy = f'3070-simpleQWAKTime_CuPy_N{
    nMin}-{nMax - 1}_P{pVal}_T{t}_S{samples}.txt'

qwak_times_file = f'Datasets/Benchmark-SimpleQWAK_ER/' + qwak_times_filename
qwak_times_file_cupy = f'Datasets/Benchmark-SimpleQWAK_ER/' + \
    qwak_times_filename_cupy
