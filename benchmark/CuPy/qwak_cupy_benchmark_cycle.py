...
def runMultipleSimpleQWAK(nList, t, samples):
    ...
    for n in nList:
        for sample in range(1, samples + 1):
            print(
                f"----> Calculating NP QWAK for n = {n} \t Sample #{sample}",
                end='\r')
...
def runMultipleSimpleQWAK_cupy(nList, t, samples):
    ...
    for n in nList:
        ...
        for sample in range(1, samples + 1):
            print(
                f"----> Calculating CuPy QWAK for n = {n} \t Sample #{sample}",
                end='\r')
...
samples = 100
qwak_times_file = f'Datasets/Benchmark-SimpleQWAK_Cycle/LINUX-simpleQWAKTime_N{
    nMin}-{nMax - 1}_T{t}_S{samples}.txt'
qwak_times_file_cupy = f'Datasets/Benchmark-SimpleQWAK_Cycle/LINUX-simpleQWAKTime_CuPy_N{
    nMin}-{nMax - 1}_T{t}_S{samples}.txt'
    ...
    try:
        ...
        subprocess.check_call(
            [
                ...
                'commit',
                '-m',
                "PYTHON: In case there are uncommited changes before running the script."])
