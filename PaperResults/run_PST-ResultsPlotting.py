...
def plot_dual_y_axis(params):
    ...
    for i, (first_half_values, second_half_values) in enumerate(
        ...
        for (
            ...
            ax1.annotate(
                label, xy=(
                    max_x_value, ax1.get_ylim()[0]), xycoords='data', xytext=(
                    0, -28), textcoords='offset points', arrowprops=dict(
                    arrowstyle="->"), horizontalalignment='center', fontsize=12)
...
save_file = os.path.join(SCRIPT_DIR, 'Output', 'PerfectStateTransfer', f'Hypercube_N{
                         len(graph2)}_T{round(t2[-1], 2)}_FROM{init}_TO{target}.png')
figsize = (8, 6)
